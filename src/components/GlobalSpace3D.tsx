...
/**
 * LE DECOR 3D, ISOLE POUR ETRE CHARGEABLE A LA DEMANDE.
 *
 * Il vivait dans `GlobalSpace.tsx`, importe en statique. Consequence : three.js,
 * @react-three/fiber et drei partaient dans le paquet de CHAQUE page, pour
 * TOUS les visiteurs — y compris les telephones, qui n'affichent jamais ce
 * decor (le garde-fou leur rend un simple degrade).
 *
 * Mesure du 02/08/2026, profil iPhone 11 avec processeur bride 4x :
 *     1 671 Ko de JavaScript compresse
 *     champ utilisable apres 3,9 s en 4G, 8,0 s en 4G lente, 15,5 s en 3G
 *
 * Un visiteur mobile telechargeait, decompressait et compilait un moteur 3D
 * qu'il n'utilisera jamais. Le sortir d'ici est la seule facon de ne pas le lui
 * envoyer : un `if` a l'execution n'empeche pas un import de partir dans le
 * paquet, seul un import dynamique le fait.
 */
...
export default function GlobalSpace3D({ starsCount, sparklesCount }: { starsCount: number; sparklesCount: number }) {
  return (
